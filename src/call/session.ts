...
export class CallSession {
  ...
  /**
   * Уходит, не прощаясь: вкладку закрывают или перезагружают.
   *
   * Прощание означало бы конец разговора для собеседника, а человек, закрывший
   * вкладку по ошибке, через десять секунд вернётся по той же ссылке. Молчание
   * оставляет ему эту возможность: собеседник уйдёт в комнату ожидания.
   *
   * Устройства при этом отпускаем сразу — индикатор записи не должен пережить
   * вкладку.
   */
  leaveQuietly(): void {
    this.teardown()
