...
function wire(): void {
  ...
  // по ошибке вкладка — повод подождать, а не расходиться.
  window.addEventListener('beforeunload', () => session?.leaveQuietly())

  // pagehide срабатывает и когда страницу просто прячут — на телефоне это
  // любое переключение в мессенджер, и обрывать разговор там нельзя. Отличает
  // одно от другого persisted: с ним страница уходит в кэш и вернётся живой,
  // без него — выгружается насовсем.
  //
  // Разбирать захват на настоящей выгрузке обязательно: фоновую вкладку
  // система выгружает сама, beforeunload при этом не срабатывает, и звуковой
  // тракт остаётся в разговорном режиме. Обычные звонки после такого идут без
  // звука, хотя микрофон уже никем не занят.
  window.addEventListener('pagehide', (event) => {
    if (event.persisted) return
    session?.leaveQuietly()
  })
}
