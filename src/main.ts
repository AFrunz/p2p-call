...
function wire(): void {
  ...
  // pagehide на телефоне срабатывает и при обычном сворачивании, а признак
  // persisted эти случаи не различает — проверено на живом устройстве. Поэтому
  // идущий разговор не трогаем ни при каких обстоятельствах: свернуть
  // приложение и вернуться должно быть безопасно.
  //
  // Разбор остаётся для всего остального: комната ожидания, обмен кодами,
  // мёртвая сессия. Там терять нечего, а закрытая вкладка не должна оставлять
  // за собой захваченный микрофон и переключённый звуковой тракт.
