 ...
 *
 * Именно websocket, а не запрос к `/health`: страница живёт на другом домене,
 * и обычный fetch упёрся бы в CORS, которого на сервере нет. Открытие сокета
 * заодно проверяет ровно то, что нужно для звонка — TLS, доступность адреса и
 * наличие эндпоинта.
 *
 * Комната при этом не создаётся: без сообщения `join` сервер о нас не узнает.
 */
