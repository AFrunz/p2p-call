...
/**
 * Одно поколение ключей для одного направления.
 *
 * `key` шифрует кадры, `chain` служит только для вывода следующего поколения.
 * Их специально разделили: ключ AES-GCM недоступен на выгрузку, а вывести из
 * него следующий пришлось бы шифрованием с фиксированным nonce — то есть
 * рискуя повтором nonce на живом потоке.
 */
