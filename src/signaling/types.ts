 ...
 * браузерами, а deflate сжимает его в 4-6 раз — иначе код было бы не переслать.
 */
export interface Envelope {
  ...
  /**
   * Момент начала проверки пар, unix-время в миллисекундах. 0 — не назначен.
   *
   * Обе стороны обязаны начать одновременно: та, что придерживает кандидатов,
   * на входящие проверки не отвечает, поэтому стартовавший в одиночку просто
   * выжигает своё окно. Общий момент кладём в код, а не считаем у каждого свой.
   */
  startAt: number
  /** Сколько секунд даётся на перенос кода. Задаёт создатель сессии. */
  holdSeconds: number
  sdp: string
}
